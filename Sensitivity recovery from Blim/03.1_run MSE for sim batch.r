...
load(file=paste(outPath,"SRmodRickeronly.RData",          sep=""))
...
load(file=paste(outPath,"resNFinal_",ifelse(short & nits<=100,100,1000),".RData",      sep=""))
load(file=paste(outPath,"resFFinal_",ifelse(short & nits<=100,100,1000),".RData",      sep=""))

if (short) 
{
try(devN<-devN100,silent=T)
try(devF<-devF100,silent=T)
devN<-devN[,,,,,1:nits]
devF<-devF[,,,,,1:nits]
rm(devN100)
rm(devF100)
...
{ 
     ...
     depleted[,ac(iYr-1)] <- ssbtp<=mpPoints$Blim 

     for (its in 1:nits)   
          {
          # to deplete the stock impose a recruitment at 75% of the historic min
          if(sum(iter(depleted,its))==0) n(biol)[1,ac(iYr-1),,,,its][] <- 1854857
          # if the stock has reached Blim start recruiting normally
          if(sum(iter(depleted,its))>0)  n(biol)[1,ac(iYr-1),,,,its][] <- B2Rbis (ssbtp[,,,,,its],SRmod[its,],its,iYr-1,devR)
          }
...
save(depleted      ,file=paste(outPath2,"/",scen,opt,mpPoints$FadultA,"_Finaldepleted.RData",     sep=""))
