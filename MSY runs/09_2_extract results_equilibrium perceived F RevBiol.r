...
load(file=paste(outPathp,"stocks.RData",         sep=""))
 ...
 scen          <- "equilPerc"              # 
 opt           <- "rev"                      # for multiple scenario combinations a counter
...
load(file=paste(outPath2,scen,opt,ac(mpPoints$Fequ),nits,"its",prlength,"yrs_res.RData",        sep=""))
...
Risk1ShortT             <- c(Risk1ShortT,res$Risk1ShortT)       # percentage of iteration that reach Blim
Risk1MidT               <- c(Risk1MidT,res$Risk1MidT)
Risk1LongT              <- c(Risk1LongT,res$Risk1LongT)

Risk2ShortT             <- c(Risk2ShortT,res$Risk2ShortT )     # percentage of iteration that reach Blim
Risk2MidT               <- c(Risk2MidT,res$Risk2MidT )
Risk2LongT              <- c(Risk2LongT,res$Risk2LongT  )

Risk3ShortT             <- c(Risk3ShortT,res$Risk3ShortT)       # percentage of iteration that reach Blim
Risk3MidT               <- c(Risk3MidT,res$Risk3MidT)
Risk3LongT              <- c(Risk3LongT,res$Risk3LongT)
...
load(file=paste(outPath2,scen,opt,nits,"its","70","yrs_resMSY.RData",        sep=""))
...
res<-data.frame(perceivedF=trueF,SSBsT=SSBsT,SSBsP=SSBsP,Yields=Yields,RecT=RecsT,RecP=RecsP,iter=iter)
res$perceivedF<-round(res$perceivedF,2)

res$perceivedF[is.na(res$perceivedF)]<-0
res$SSBsP[res$perceivedF==0]<-res$SSBsT[res$perceivedF==0]
...
png(file=paste(outPath2,"plotsRevbiol/plot_equilibrium_and_risk1000its",prlength2,"_",nYrMean,".png",sep=""),width=12,height=4,units="in",res=200)
...
png(file=paste(outPath2,"plotsRevbiol/densityMSY_and_risks",prlength2,"_",nYrMean,".png",sep=""),width=8,height=8,units="in",res=200)
...
lines(risk$perceivedF,risk$Risk1LongT/100,lty="dashed",col="green",lwd=2)
lines(risk$perceivedF,risk$Risk2LongT/100,lty="dotted",col="green",lwd=2)
lines(risk$perceivedF,risk$Risk3LongT/100,lty="solid",col="green",lwd=2)
abline(h=0.05,lty="dashed",col="green")
text(0.07,0.05,"5% risk",pos=3,col="green")
abline(h=0)
abline(v=FmsyprecLongT1,col="green",lty="dashed")
abline(v=FmsyprecLongT2,col="green",lty="dotted")
abline(v=FmsyprecLongT3,col="green",lty="solid")

legend(x="topleft",bty="n",col=c("red",rep("green",3)),lty=c("solid","dashed","dotted","solid"),c("Fmsy distrib","LongT risk1","LongT risk2","LongT risk3"))
...
png(file=paste(outPath2,"plotsRevbiol/otherRefPoints",prlength2,"_",nYrMean,".png",sep=""),width=8,height=8,units="in",res=200)
