...
#  iBiol= biol[,ac(projPeriod)] ; iHarvest =f[,ac(projPeriod)]   ;iStck=stocks[,ac(projPeriod)]
...
# yr    <-  iYr-1
          ...
          idxsegreg<- which(mod=="segreg")
          idxricker<- which(mod=="ricker")
          idxbevholt<- which(mod=="bevholt")
          mu    <- numeric(length(A))
          mu[idxsegreg] <- A[idxsegreg]*(Biom[idxsegreg]>=B[idxsegreg])+A[idxsegreg]*Biom[idxsegreg]*(Biom[idxsegreg]<B[idxsegreg])/B[idxsegreg]
          mu[idxricker] <- A[idxricker]*Biom[idxricker]*exp(-B[idxricker]*Biom[idxricker])
          mu[idxbevholt] <- A[idxbevholt]*Biom[idxbevholt]/(B[idxbevholt]+Biom[idxbevholt])
          REC        <- (mu * exp(dev[,ac(yr)]))+10
