...
#outPath<-paste(outPathp,sc.name,"/",sep="")

outPath<-paste(outPath,"test/",sep="")
...
load(file=paste(outPath,"/",scen,"_Finalsettings.RData",           sep=""))
load(file=paste(outPath,"/",scen,"_FinalSSB.RData",           sep=""))
source(paste(codePath,"functions.r",            sep=""))
source(paste(codePath,"04_forecastScenarios.r", sep=""))
...
# error on real scale
devSSB<-(ssb(stockstore)[,projPeriod]-ssbb(biol,f,stocks)[,projPeriod])
...
# error on log scale
devSSB<-log(ssb(stockstore)[,projPeriod]/ssbb(biol,f,stocks)[,projPeriod])
...
savePlot(file=paste(plotPath2,"2.logSSBinTaY_error.png",sep=""),type="png"); dev.off()
...
savePlot(file=paste(plotPath2,"3.RHO_logSSBinTaYerrors.png",sep=""),type="png"); dev.off()
...
savePlot(file=paste(plotPath2,"4.logSSBinAdY_error.png",sep=""),type="png"); dev.off()

# autocor error in advice year on log scale
rhoSSBdev<-rep(NA,nits)
for (i in 1:nits) rhoSSBdev[i]<- an(unlist(acf(iter(devSSB,i),plot=F)[1])[1])
histogram(1~rhoSSBdev,col="grey",main="RHO errors logSSB(AdY)")
savePlot(file=paste(plotPath2,"5.RHO_logSSBinAdYerrors.png",sep=""),type="png"); dev.off()
...
savePlot(file=paste(plotPath2,"6.logFbarinTaY_error.png",sep=""),type="png"); dev.off()

histogram(1~rhoFdev,col="grey",main="RHO errors logFbar(TaY)")
savePlot(file=paste(plotPath2,"7.RHO_logFbarinTaYerrors.png",sep=""),type="png"); dev.off()
...
savePlot(file=paste(plotPath2,"12.TAC.png",sep=""),type="png");dev.off()
...
savePlot(file=paste(plotPath2,"13.iterations.png",sep=""),type="png");dev.off()
...
savePlot(paste(plotPath2,"14.Truth.png",sep=""),type="png") ; dev.off()
