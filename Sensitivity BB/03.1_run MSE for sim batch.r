...
outPath2<-paste(outPath,"/HCR sensitivity BB/",sc.name,"/",sep="")
...
#  stockstore[,ac(TaY)]@catch.wt  <-   stocks[,ac(TaY)]@catch.wt
 stockstore[,ac(TaY)]  <-   stocks[,ac(TaY)]    
  ...
  #- Project 
  res                       <- projectMac(stocks[,1:idxyrmin1],survivors,window(fishery,histMinYr,yrmin1),iYr,TAC,mpPoints$scen,NULL,histMaxYr,mpPoints,mpOptions,BB)
  TAC[,ac(FcY)]             <- res[["TAC"]]
  HCRTAC[,ac(FcY)]          <- res[["HCRTAC"]]
  HCRSSB[,ac(FcY)]          <- res[["SSB"]][["HCRSSB"]][,ac(FcY)]
  SSB[,ac(FcY)]             <- res[["SSB"]][["SSB"]][,ac(FcY)]
  BB[,ac(FcY)]              <- res[["BB"]][,ac(FcY)]
...
save(BB           ,file=paste(outPath2,"/",scen,opt,mpPoints$FadultA,"_FinalBB.RData",        sep=""))
