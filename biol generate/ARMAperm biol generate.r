...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 x<-(x-mean(x))/sd(x)
 x<-0.8*sd(dats)*x+mean(rev(dats)[1:3])         # THAT'S where I specify that the recent mean should be used as a base level
...
save(wt,file=paste(inPath,"ARMApermstock.wt.RData"))
...
save(cw,file=paste(inPath,"ARMApermcatch.wt.RData"))
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 # standardise the series : same mean and var as the historical series
 x<-(x-mean(x))/sd(x)
 x<-0.8*sd(dats)*x+mean(rev(dats[1:3]))
...
save(mat,file=paste(inPath,"ARMApermmat.RData"))
...
plot(mprop)
save(mprop,file=paste(inPath,"ARMApermmprop.RData"))
...
save(fprop,file=paste(inPath,"ARMApermfprop.RData"))
