...
load(file=paste(outPath,"refsel.RData",        sep=""))
...
source(paste(codePath,"Sensitivity ref selection pattern/04_forecastScenarios.r", sep=""))
...
outPath2<-paste(outPath,"HCR sensitivity ref selection pattern/",sc.name,"/",sep="")
...
#   
  ...
  #- Project 
  res                       <- projectMacrefsel(stocks[,1:idxyrmin1],survivors,window(fishery,histMinYr,yrmin1),iYr,TAC,mpPoints$scen,NULL,histMaxYr,mpPoints,mpOptions,ref.selpat)
  TAC[,ac(FcY)]             <- res[["TAC"]]
