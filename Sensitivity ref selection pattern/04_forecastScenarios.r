...
projectMacrefsel <- function(iStocks,iSurvivors,iFishery,iYr,iTAC,iScenario,iFailRuns=NULL,iHistMaxYr,mpPoints,mpoptions,refesel){
...
# refesel<- ref.selpat
  ...
  # modify the selection pattern to use the reference one
  
 stf@harvest[,FcY] <-refesel
  ...
  if(mpoptions$BBscen == "Banking"){
    ...
    if(an(iYr) >= (iHistMaxYr+1)){             #Repay banked part                               Bank
