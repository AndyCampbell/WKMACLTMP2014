...
load(file=paste(outPathp,"stocks.RData",         sep=""))
 ...
 scen          <- "equilPerc"              # 
 opt           <- "RefSel"                      # for multiple scenario combinations a counter
...
fs<- c(seq(0.0,0.29,0.01),c(0.31,0.50,0.01)
fs<- seq(0.32,0.50,0.01)
for (fequ in fs)
{          
...
save.image(file=paste(outPath2,"plots sensitivity RefSel/",scen,opt,nits,"its",prlength2,"yrs_resMSY.RData",        sep=""))
}
...
load(file=paste(outPath2,"plots sensitivity RefSel/",scen,opt,nits,"its",prlength2,"yrs_resMSY.RData",        sep=""))
...
iter<-rep(1:nits,length(seq(0,0.5,0.01))-1)
...
png(file=paste(outPath2,"plots sensitivity RefSel/plot_equilibrium_and_risks",prlength2,"_",nYrMean,".png",sep=""),width=12,height=4,units="in",res=200)
...
png(file=paste(outPath2,"plots sensitivity RefSel/densityMSY_and_risks",prlength2,"_",nYrMean,".png",sep=""),width=8,height=8,units="in",res=200)
...
png(file=paste(outPath2,"plots sensitivity RefSel/otherRefPoints",prlength2,"_",nYrMean,".png",sep=""),width=8,height=8,units="in",res=200)
