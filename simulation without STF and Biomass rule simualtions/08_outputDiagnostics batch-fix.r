...
diags<-data.frame(scenario=NA,RecRegime=NA,Iterations=NA,Btrigger=NA,Blim=NA,alpha=NA,
      ...
      meanrelTACIAV=NA,meanrelTACIAV_NA=NA,noIAVrestrictup=NA,noIAVrestrictdown=NA,TACup=NA,TACdown=NA,
...
for (sc in c("Alpha","2mt","","2.4mt","2.6mt","2.8mt","3.0mt","3.2mt"))
{
for (opt in 1:21)
{
...
sc.name<-paste(scen,opt,"_TACvarlim",TACvarlim,"_Fvarlim",Fvarlim,"_",BBscen,"_LastRec",LastRecOp,"_fix.prop",sep="")

outPath2<-paste("R code/GRID-RESULTS-1000/perm",sc.name,sep="")
source(paste(codePath,"07_scenarioDescription_modified_fix.r", sep=""))
...
diags[counter,"Risk2ShortT"]             <- length(unique(which(Ssb[,ShortT]<Blim,arr.ind=T)[,"iter"]))/dims(Ssb)$iter  *100       # percentage of iteration that reach Blim
diags[counter,"Risk2MidT"]               <- length(unique(which(Ssb[,MidT]  <Blim,arr.ind=T)[,"iter"]))/dims(Ssb)$iter  *100
diags[counter,"Risk2LongT"]              <- length(unique(which(Ssb[,LongT] <Blim,arr.ind=T)[,"iter"]))/dims(Ssb)$iter  *100
...
landings.null <- iter((quantSums(fishery@landings.n))[,35:73],1)
landings.null[] <- NA
ymw.null <- landings.null
SSB.null <- landings.null
for(iii in 1:39){
landings.null[,iii] <- length(which(c(quantSums(fishery@landings.n)[,35+iii])==0))
ymw.null[,iii] <- length(which(is.na(c(ymw[,35+iii]))))
SSB.null[,iii] <- length(which(c(Ssb[,35+iii])==0)) 
}
...
diags[counter,"meanrelTACIAV_NA"]        <- round(median(c(apply(abs(TAC[,ac(projPeriod[2]:rev(projPeriod)[1])] - TAC[,ac(projPeriod[1]:rev(projPeriod)[2])]) /TAC[,ac(projPeriod[1]:rev(projPeriod)[2])] * 100,3:6,mean))),3)
...
if((nrow(IAVUp)) > 0 ){
  a <- IAVUp
  diags[counter,"noIAVrestrictup"]    <- max(0,median(aggregate(a[,"year"],by=list(a[,"iter"]),function(x){length(x)})$x),na.rm=T)
}
 diags[counter,"noIAVrestrictdown"]<-0
if((nrow(IAVDown)) > 0 ){
  a <- IAVDown
  diags[counter,"noIAVrestrictdown"]  <- max(0,median(aggregate(a[,"year"],by=list(a[,"iter"]),function(x){length(x)})$x),na.rm=T)
}
#
...
write.csv(t(diags),file=paste(outPath,"/tables_diags",paste("_TACvarlim",TACvarlim,"_Fvarlim",Fvarlim,"_",BBscen,"_LastRec",LastRecOp,"_fix.prop",sep=""),".csv",sep=""),row.names=T)

#-------- #
scen <- "LTMP3.2mt"
opt <- 21
opt <-10
var <- c("Risk1ShortT","Risk1MidT", "Risk1LongT", "Risk2ShortT","Risk2MidT","Risk2LongT",
         "Risk3ShortT","Risk3MidT","Risk3LongT",
         "meanYieldShortTerm","meanYieldMidTerm","meanYieldLongTerm","meanrelTACIAV","meanrelTACIAV_NA",
         "meanSSBShortT","meanSSBMidT","meanSSBLongT",
         "YmeanWeightShortT","YmeanWeightMidT","YmeanWeightLongT")


dim1 <- 21#length(unique(diags$alpha))
dim2 <- length(unique(diags$Btrigger))


for(kk in 1: length(var)){
  data.Med <- matrix(NA, ncol=dim1,nrow=dim2)
  
  for(j in 1: dim2){
    B.trig <- sort(unique(diags$Btrigger))[j]
    diags.sub <- diags[diags$Btrigger==B.trig,]
    data.Med[j,] <- t(as.vector(diags.sub[var[kk]]))
  }
  
  
  data.Med.df <- as.data.frame(data.Med)
  colnames(data.Med.df) <- sort(unique(diags$alpha))
  rownames(data.Med.df) <- sort(unique(diags$Btrigger))
  
  write.csv(data.Med.df,file=paste(outPath,"/tables_diags",var[[kk]], "_fix.prop.csv",sep=""),row.names=T)
}
