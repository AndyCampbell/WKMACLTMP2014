...
for (sc in c("2mt","","2.4mt","2.6mt","2.8mt","3.0mt","3.2mt"))
{
for (opt in 1:20)
{
...
write.csv(t(diags),file=paste(outPath,"/tables_diags",paste(scen,opt,"_TACvarlim",TACvarlim,"_Fvarlim",Fvarlim,"_",BBscen,"_LastRec",LastRecOp,"_wstf",sep=""),".csv",sep=""),row.names=T)
...
var <- c("Risk1ShortT","Risk1MidT", "Risk1LongT", "Risk2ShortT","Risk2MidT","Risk2LongT",
         ...
         "meanYieldShortTerm","meanYieldMidTerm","meanYieldLongTerm","meanrelTACIAV",
         "meanSSBShortT","meanSSBMidT","meanSSBLongT",
         "YmeanWeightShortT","YmeanWeightMidT","YmeanWeightLongT")


dim1 <- length(unique(diags$Ftarget))
dim2 <- length(unique(diags$Btrigger))
  ...
  colnames(data.Med.df) <- sort(unique(diags$Ftarget))
  rownames(data.Med.df) <- sort(unique(diags$Btrigger))

  write.csv(data.Med.df,file=paste(outPath,"/tables_diags",var[[kk]], "_wstf.csv",sep=""),row.names=T)
 }
