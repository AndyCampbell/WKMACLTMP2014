...
load(file=paste(outPathp,"ctch.RData",          sep=""))
load(file=paste(outPathp,"surveys.RData",       sep=""))
...
scen          <- c("LTMP")              # 
opt           <- 4                      # for multiple scenario combinations a counter
...
for (iYr in an(projPeriod)){
  ...
  n(biol)[ac((range(biol,"min")+1):range(biol,"max")),ac(iYr),,] <- survivors[-dim(survivors)[1],,,,,]@.Data
     ...
     cat("replace this function with the one from the Jack Mackrel")
     n(biol)[1,ac(iYr)] <- B2RF (ssbtp,SRmod,iYr,RecType)
     cat("compare the output")
