...
scen          <- c("LTMP2.2mt")              # 
opt           <- 15                      # for multiple scenario combinations a counter
...
outPath2<-paste(outPath,"test/",sep="")
...
#   
  ...
  stockstore@stock.n[,ac(TaY)] <-    stocks@stock.n[,ac(TaY)]
  stockstore@harvest[,ac(TaY)]  <-    stocks@harvest[,ac(TaY)]
  stockstore@catch.wt[,ac(TaY)]  <-   stocks@catch.wt[,ac(TaY)]
