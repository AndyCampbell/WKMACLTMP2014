...
nits<-750
its <- sample(1:1000,nits,replace=F)
diags[counter,"repl"]<-repl
biol<-biol[,,,,,its]
f<- f[,,,,,its]
stockstore <- stockstore[,,,,,its]
...
write.csv((diags),file=paste(outPath,"/risk vs iters/tables_diags_HCR base caser",nits,".csv",sep=""),row.names=T)
