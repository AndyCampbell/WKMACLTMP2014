...
projectMac <- function(iStocks,iSurvivors,iFishery,iYr,iTAC,iScenario,iFailRuns=NULL,iHistMaxYr,mpPoints,mpoptions,ibb){
...
#ibb<-BB
  ...
  #- Scenario no IAV
  #-----------------------------------------------------------------------------
  if(mpoptions$TACvarlim == F | ImY==ac(iHistMaxYr+2) ){
  ...
  #-----------------------------------------------------------------------------
  if(mpoptions$TACvarlim == T & ImY!=ac(iHistMaxYr+2)){
    iTAC[,FcY]                <- HCRTAC
...
##############################################################################
##     BB scheme stops applying if SSb <Bpa two years in a row 
#                                     or Fbar>Fpa and SSB<Bpa
#
     Bpa   <-  2.36e6
     Fpa   <-  0.31
#
#
  ...
  if(mpoptions$BBscen == "BB"){
    ...
    if(iYr == ac(iHistMaxYr+1)){             #Bank
      ibb[,FcY]         <-    -  0.1 * iTAC[,FcY]
     iTAC[,FcY]         <-  iTAC[,FcY] + ibb[,FcY] -ibb[,ImY]

    }
    if(iYr == ac(iHistMaxYr+2)){             #Repay banked part                               Borrow
     ibb[,FcY]     <-      0.1 * iTAC[,FcY]
     iTAC[,FcY]    <-  iTAC[,FcY] + ibb[,FcY] -ibb[,ImY]

    }
    if(an(iYr) > (iHistMaxYr+2)){            #Repay borrowed part                             Borrow
     ibb[,FcY]     <-      0.1 * iTAC[,FcY]
     iTAC[,FcY]    <-  iTAC[,FcY] + ibb[,FcY] -ibb[,ImY]
    ...
    stf@harvest[,FcY]         <- fleet.harvestFF(stk=stf,iYr=FcY,TACS=iTAC[,FcY])
    ssb                       <- quantSums(stf@stock.n[,ac(TaY:ImY),1]*stf@stock.wt[,ac(TaY:ImY),1]*stf@mat[,ac(TaY:ImY),1]*
                                           exp(-unitSums(stf@harvest[,ac(TaY:ImY)])*stf@harvest.spwn[,ac(TaY:ImY),1]-stf@m[,ac(TaY:ImY),1]*stf@m.spwn[,ac(TaY:ImY),1]))
    Fb                        <- quantMeans(harvest(stocks)[ac(4:8),ac(ImY)]) 
   

    crit                      <- (ssb[,ac(ImY)]<Bpa) & (ssb[,ac(TaY)]<Bpa | Fb>Fpa)                                       
    idxssb                    <- which(crit)
    if(length(idxssb)>0) cat("stop BB for",length(idxssb),"its in",ImY,"\n")
    if(length(idxssb)>0) iTAC[,FcY,,,,idxssb] <- HCRTAC[,,,,,idxssb]  -ibb[,ImY,,,,idxssb]
  ...
  if(mpoptions$BBscen == "Banking"){
    ...
    crit                      <- (ssb[,ac(ImY)]<Bpa) & (ssb[,ac(TaY)]<Bpa | Fb>Fpa)                                       
    idxssb                    <- which(crit)
    if(length(idxssb)>0) cat("stop Banking for",length(idxssb),"its in",ImY,"\n")        
    if(length(idxssb)>0) iTAC[,FcY,,,,idxssb] <- HCRTAC[,,,,,idxssb]  -ibb[,ImY,,,,idxssb]
  }
  ...
  if(mpoptions$BBscen == "Borrowing"){
    ...
    #- first year of  banking
    if(iYr == ac(iHistMaxYr+1)){             #Borrow
     ibb[,FcY]     <-      0.1 * iTAC[,FcY]
     iTAC[,FcY]         <-  iTAC[,FcY] + ibb[,FcY] -ibb[,ImY]

    }
    if(an(iYr) >= (iHistMaxYr+1)){             #Repay                   and          Borrow again
     ibb[,FcY]     <-      0.1 * iTAC[,FcY]
     iTAC[,FcY]         <-  iTAC[,FcY] + ibb[,FcY] -ibb[,ImY]
    }
    ...
    crit                      <- (ssb[,ac(ImY)]<Bpa) & (ssb[,ac(TaY)]<Bpa | Fb>Fpa)                                       
    idxssb                    <- which(crit)
    if(length(idxssb)>0) cat("stop Borrowing for",length(idxssb),"its in",ImY,"\n")                
    if(length(idxssb)>0) iTAC[,FcY,,,,idxssb] <- HCRTAC[,,,,,idxssb]  -ibb[,ImY,,,,idxssb]
  }
  ...
  #-----------------------------------------------------------------------------
  if(mpoptions$BBscen == "AlternateBank"){
    ...
    al<-(an(iYr)%%2-0.5)/5          # in (iHistMaxYr+2) al = - 0.1
    ...
    crit                      <- (ssb[,ac(ImY)]<Bpa) & (ssb[,ac(TaY)]<Bpa | Fb>Fpa)                                       
    idxssb                    <- which(crit)    
    if(length(idxssb)>0) cat("stop Alt Borrow for",length(idxssb),"its in",ImY,"\n")        
    if(length(idxssb)>0) iTAC[,FcY,,,,idxssb] <- HCRTAC[,,,,,idxssb]  -ibb[,ImY,,,,idxssb]
  }
  ...
  if(mpoptions$BBscen == "AlternateBorrow"){
    ...
    crit                      <- (ssb[,ac(ImY)]<Bpa) & (ssb[,ac(TaY)]<Bpa | Fb>Fpa)                                       
    idxssb                    <- which(crit)    
    if(length(idxssb)>0) cat("stop alt Banking for",length(idxssb),"its in",ImY,"\n")        
    if(length(idxssb)>0) iTAC[,FcY,,,,idxssb] <- HCRTAC[,,,,,idxssb]  -ibb[,ImY,,,,idxssb]
  }
...
#for all years y? (iHistMaxYr+1), where the "sign" function is defined as:
#sign(x) = 1 if x>0;  = 0 if x=0; = -1 if x>0.
#

    borrow <- - sign(iTAC[,FcY,]-iTAC[,ImY,]) *         apply(abs(iTAC[,FcY,]-iTAC[,ImY,])/iTAC[,ImY,],c(6),function(x) {min(0.1,x)})
    # borrow>0 = borrow in year Fcy from next year
    # borrow<0 = bank   in year Fcy for  next year
    ibb[,FcY] <- borrow*iTAC[,FcY]
    #-  HCR with 15% IAV
    iTAC[,FcY]                <- iTAC[,FcY] + ibb[,FcY] -ibb[,ImY] 
    #- Make sure no SSB is below Blim
    stf@harvest[,FcY]         <- fleet.harvestFF(stk=stf,iYr=FcY,TACS=iTAC[,FcY])
...
res<-list(TAC=iTAC[,ac(FcY)],HCRTAC=HCRTAC[,ac(FcY)],SSB=list(HCRSSB=HCRSSB[,ac(FcY)],SSB=SSB[,ac(FcY)]),fSTF=unitSums(stf@harvest[,FcY]),BB=ibb)
  return(res)
